...
export const pilotCandidates: PilotCandidate[] = [
  ...
  },
  {
    ...
    age: 45, education: "Graduate", profession: "Politician", totalAssets: "Illustrative — pending verification", criminalCases: 0,
...
export function getCandidatesByPartyAndState(partyAbbr: string, state: string) {
  return pilotCandidates.filter((c) => c.partyAbbr === partyAbbr && c.state === state).sort((a, b) => (a.sortOrder ?? 99) - (b.sortOrder ?? 99));
}
...
export function getOtherCandidatesInState(partyAbbr: string, state: string) {
  ...
  return Array.from(grouped.entries()).map(([abbr, candidates]) => ({
    partyAbbr: abbr, partyName: candidates[0].party,
